...
var functions = {
     ...
     */
    password: function() {
        return functions.callApi(_MIDEA_COMMON, 'authPassword')
    },
    /**
     * 显示菜单
     * @return {*|promise}
        ...
        return new Promise(function(resolve, reject) {
            if (navigator.camera) {
                try {
                    ...
                    cameraOptions.sourceType = params.sourceType; // 设置打开相机还是相册 'Camera.PictureSourceType.PHOTOLIBRARY' 
                    cameraOptions.destinationType = params.destinationType; // 返回的图片数据格式  FILE_URI    DATA_URL
                    cameraOptions.allowEdit = false; // 是否允许编辑
                    cameraOptions.quality = params.quality || 100; // 照片压缩比例 [1, 100]
                    ...
                    cameraOptions.encodingType = params.encodingType
     ...
     * @return {*|promise}
     */
    showWidget: function(id, params) {
        return functions.callApi(_MIDEA_COMMON, 'showWidget', [id, params])
    },
    /**
     * 显示键盘
     * @return {*|promise}
     ...
     * @params：
     */
    saveToGallery: function(params) {
        return functions.callApi(_MIDEA_COMMON, 'saveToGallery', [params])
    },
    /**
     * 美信分享
     * @params: {'title': '分享标题_test',
                      'subTitle': '分享子标题',
                      'action': 'openH5',
                      'widgetId': 'com.midea.demo',
                      'widgetExtra': '{"key1":"value1","key2":"value2"}',
                      'url': 'http://www.baidu.com',
                      'imageUrl': 'http://pic.nipic.com/2007-11-09/2007119122519868_2.jpg',
                      'actionType': '1'
                    }
     */
    mcShare: function(params) {
        return functions.callApi(_MIDEA_COMMON, 'mcShare', [params])
    }
}
