<template>
  <div class="MaintainCalcu">
    ...
    <div class="conf">
      ...
      <div class="conf-box" v-for="(item, index) in carObj.aegis_claim" :key="index">
        <span class="conf-attr">{{item.name}}：</span>
        <span class="conf-data2">{{item.content}}</span>
...
export default {
  ...
  mounted(){
    console.log('toute,',window.location.href);
    this.init()
  },
  methods: {
    // ...mapActions(["setTab"]),
    ...
    async init(){
      ...
      let data = await this.api.aegisClaim({
        token:this.$route.query.token,
        user_car_id:this.$route.query.carId
      })
      this.carObj = data
      console.log(' E50 单独保养计算------',data);
