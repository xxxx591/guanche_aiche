<template>
  <div class="ShopCar">
    <div class="top">
      ...
      <div class="top-collect" @click.stop="$router.push({name: 'EditShopCar'})">编辑</div>
...
export default {
  ...
  methods: {
    ...mapActions(['addShopCar', 'selectedShopCar', 'addProductBuyNum', 'subProductBuyNum', 'toggleIsSelectedAll']),
    inputBlur() {
      window.scrollTo(0, 0)
    },
    async getAllShopCarProducts() {
      let token = await this.native.getToken({})
      let list = await this.api.shopCarList({ token: token.token, page: 1, pagesize: 100000 })
