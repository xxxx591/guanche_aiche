  ...
  <div class="EditAddr">
    ...
    <div class="box flex-v" @click.stop="isShowAddrBox = !isShowAddrBox">
      ...
      <div class="box-input1 flex-h">
        ...
        <img src="../../assets/images/right-arrow.png" alt class="box-img">
      </div>
    </div>
    <div class="box flex-v">
...
export default {
  ...
  created() {
    console.log('addr----222', this.$route.params.addr)
    this.userAddr = { province: this.$route.params.addr.province, city: this.$route.params.addr.city, area: this.$route.params.addr.area }
    this.userName = this.$route.params.addr.cnee
    this.phone = this.$route.params.addr.mobile
    this.detailAddr = this.$route.params.addr.address
    this.oldAddr = this.$route.params.addr
  },
  methods: {
    tempMove() { },
    tempClick() { },
    ...
    selectAddr(addr) {
      ...
      this.userAddr = { province: addr.province.value, city: addr.city.value, area: addr.area.value }
    },
    async deleteAddr() {

      let token = await this.native.getToken({})
      let result = await this.api.deleteAddr({ token: token.token, address_id: this.oldAddr.id })
    ...
    async saveAddr() {
      ...
      let token = await this.native.getToken({})
      let result = await this.api.updateAddr({
        address_id: this.oldAddr.id,
        cnee: this.userName,
        mobile: this.phone,
