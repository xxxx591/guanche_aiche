<template>
  <div class="ConfirmOrder">
    ...
    <div class="product flex-h">
      <div class="p-left">
        <img src="../../assets/temp-car.png" alt class="p-img">
      </div>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      if (
        ...
        from.name == "QualityProductDetail"
      ) {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      let result = await this.api.submitOrder_jiFen({
        token: token.token,
        point_id: this.product.id,
        number: this.buyNum,
        address_id: this.addr.id
      });
      if (result.error_code === 0) {
        this.$router.push({ name: "ConfirmBack", query: { type: 2 } });
      }
    },
