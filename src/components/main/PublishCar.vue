  ...
  <div class="PublishCar">
    ...
    <div class="pic">
      ...
      <div class="pic-img flex-h">
        <div class="pic-box" v-for="(item, index) in imgArr" @click.stop="deleteImg(item) " :key="index">
  ...
  methods: {
    ...
    async publishArticle() {
      ...
      let result = await this.api.publishSecondCar({
        ...
        load: parseInt(this.form.selectedCarWeight),
        course: parseInt(this.form.carDistance),
        plate_time: parseInt(this.form.carCardTime),
    ...
    async articleAddImg() {
      ...
      result.image.forEach((item, index) => {
        this.pasteHtmlAtCaret(`<img style="width:100%;margin: 0 auto;display: block;" src="${item}"><br/>`)
      })
